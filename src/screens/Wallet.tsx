...
export default function Wallet() {
  const [step, setStep]         = useState<'loading' | 'welcome' | 'backup' | 'restore' | 'home'>('loading');
  const [address, setAddress]   = useState('');
  const [mnemonic, setMnemonic] = useState('');
  const [inputPhrase, setInput] = useState('');
  ...
  // load mnemonic when we hit the backup screen
  useEffect(() => {
    if (step === 'backup') {
      (async () => setMnemonic((await getStoredMnemonic()) ?? ''))();
    }
  }, [step]);
      ...
      <View style={styles.container}>
        ...
        <Button title="Create New Wallet" onPress={async () => {
          try {
            ...
            setAddress(addr); setStep('backup');
  ...
  if (step === 'backup')
    return (
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <Text style={styles.subtitle}>Write down these 24 words in order and keep them safe.</Text>
        <Button title="I’ve backed it up — show my address" onPress={() => setStep('home')}/>
      </ScrollView>
    );
  ...
  if (step === 'restore')
    return (
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <Text style={styles.subtitle}>Paste your 24-word backup phrase below.</Text>
        <TextInput
          style={styles.input} multiline
          placeholder="word1 word2 … word24"
          value={inputPhrase} onChangeText={setInput}
        />
        <Button title="Restore Wallet" onPress={async () => {
          try {
            const words = inputPhrase.trim().split(/\s+/);
            if (words.length !== 24) throw new Error('Please enter exactly 24 words.');
            const addr = await restoreWalletFromMnemonic(inputPhrase);
            setAddress(addr); setStep('home');
          } catch (e: any) { setError(e.message); }
        }}/>
      </ScrollView>
    );
  ...
  return <View style={styles.container}><Text style={styles.subtitle}>Loading…</Text></View>;
}
