...
// small, paced fetcher to be polite with CG
...
const Wallet = () => {
  ...
  // CoinGecko enrichment for 24h %
  const [cgMap, setCgMap] = useState<Record<string, CGMarket>>({});
  const resolving = useRef<Set<string>>(new Set()); // avoid duplicate lookups
  ...
  // Preload top-200 market map (fast path)
  useEffect(() => {
    (async () => {
  ...
  // Lazy resolver when a symbol isn't in the map (slow path, once per symbol)
  ...
  const filteredBalances = balances.filter(
    (item) =>
      Number(ethers.utils.formatEther(item.balance)) > 0 &&
  ...
  const renderBalanceItem = ({ item }: { item: any }) => {
    let adjustedBalanceWei = item.balance;
    if (item.contract_ticker_symbol === 'ETH') {
      const originalEth = ethers.utils.formatEther(item.balance);
      const adjustedEth = (parseFloat(originalEth || '0') + localBalanceDelta).toString();
      adjustedBalanceWei = ethers.utils.parseEther(isFinite(Number(adjustedEth)) ? adjustedEth : '0').toString();
    }
    const adjustedEth = Number(ethers.utils.formatEther(adjustedBalanceWei));
    const balanceLine = `${adjustedEth.toFixed(8)} ${item.contract_ticker_symbol}`;
    ...
    // If no map row, kick off a lazy resolve (once) so the % shows shortly after
  ...
  return (
    <View style={styles.container}>
      ...
      {/* Labeled pickers with visible value + centered headings */}
      ...
      {loading ? (
        ...
        <FlatList
          ...
          keyExtractor={(item) =>
            (item.contract_address ? String(item.contract_address) : '') +
            (item.token_id ? String(item.token_id) : String(Math.random()))
          }
...
const styles = StyleSheet.create({
  ...
  // Pickers with visible display (centered labels)
  ...
  // Cards (bigger, to match Search) — updated shadow
  assetList: { flex: 1 },
  card: {
    flexDirection: 'row',
    alignItems: 'center',
  ...
  // price on the right + 24h% underneath
