 ...
 import { getWalletAddress } from './src/utils/wallet'; // Added for loadAddress

 export default function App() {
   const { setAuthenticated, setHasMnemonic, setHasPin } = useAuthStore();
   const setAddress = useWalletStore((state) => state.setAddress); // Added for setAddress
   ...
   useEffect(() => {
     const checkAuth = async () => {
       try {
         ...
         if (hasMn && hasP) {
           const currentAddress = await getWalletAddress(); // Added: Load address
